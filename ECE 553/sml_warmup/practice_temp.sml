...
struct
    ...
    (* Helper function to update the map with a word occurrence *)
    fun updateMap (map, word, file) =
        case M.find(map, word) of
            SOME set => M.insert(map, word, S.add(set, file))
          | NONE => M.insert(map, word, S.add(S.empty, file))

    (* Helper function to process a single file *)
    fun processFile (filename, map) =  (* Changed parameter order here *)
        let
            val contents = readFile filename
            val words = splitIntoWords contents
            fun addWord (word, accMap) = updateMap(accMap, word, filename)
        in
            foldl addWord map words
        end

    (* Main processing function *)
    fun proc filenames =
        foldl processFile M.empty filenames
end

(* Example instantiation using built-in structures *)
