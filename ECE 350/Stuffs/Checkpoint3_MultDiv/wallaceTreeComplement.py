...
output [''' + str((inputLen)-1) + ''':0] data_result;
output data_exception, data_resultRDY;

wire [''' + str((2*inputLen)-1) + ''':0] data_result;
...
for bit in range(0, inputLen): # for each bit 
    if (bit < inputLen+0):
...
assignOutputStr += "assign w_0_31 = !(data_operandA[0] & data_operandB[31]);\n"
...
for bit in range(row, inputLen+row): # for each bit 
    ...
    if (bit == inputLen+row-1):
        fullAdderStr += "full_adder add_" + wStr + "(.S(" + wStr + "), .Cout("+ "w_" + str(row) + "_" + str(inputLen+row) + "),  .A(1'b1), .B( !(data_operandA[" + str(row) + "] & data_operandB[" + str(bit-row) + "])), .Cin(" + "c_" + str(row) + "_" + str(bit-1) + "));\n"
    elif (bit != row):
...
for row in range(2, inputLen-1): # for each row (ie each bit of Y)
...
for bit in range(row, inputLen+row): # for each bit 
    ...
    if (bit == inputLen+row-1):
        fullAdderStr += "full_adder add_" + str(bit) + "(.S(" + wStr + "), .Cout("+ "w_" + str(row) + "_" + str(inputLen+row) + "),  .A(" + "w_" + str(row-1) + "_" + str(bit) + "), .B((data_operandA[" + str(row) + "] & data_operandB[" + str(bit-row) + "])), .Cin(" + "c_" + str(row) + "_" + str(bit-1) + "));\n"
    elif (bit != row):
        fullAdderStr += "full_adder add_" + str(bit) + "(.S(" + wStr + "), .Cout("+ cStr + "),  .A(" + "w_" + str(row-1) + "_" + str(bit) + "), .B( !(data_operandA[" + str(row) + "] & data_operandB[" + str(bit-row) + "])), .Cin(" + "c_" + str(row) + "_" + str(bit-1) + "));\n"
    else:
        fullAdderStr += "full_adder add_" + str(bit) + "(.S(" + wStr + "), .Cout("+ cStr + "),  .A(" + "w_" + str(row-1) + "_" + str(bit) + "), .B(!(data_operandA[" + str(row) + "] & data_operandB[" + str(bit-row) + "])), .Cin(1'b0));\n"
...
for zIndex in range(inputLen, outputLen-1):
...
assignOutputStr += "assign data_result[" + str(zIndex) + "] = w_" + str(inputLen-1) + "_" + str(zIndex) + ";\n"
print("wire throwaway;")
assignOutputStr += "full_adder add_final_bit(.S(data_result[" + str(outputLen-1) + "]), .Cout(throwaway),  .A(1'b1), .B(1'b0), .Cin(w_31_63));\n"
